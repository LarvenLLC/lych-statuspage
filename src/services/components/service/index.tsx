...
const ServiceItem: FunctionComponent<ServiceItemProps> = ({ item }) => {
    const Icon = () => {
        if (item?.status === Status.OPERATIONAL) {
            return <svg className="h-6 w-6 flex-none fill-sky-100 stroke-green-500 dark:stroke-green-900 stroke-2">
        ...
        <div className='mb-10'>
            <div className='flex'>
                ...
                <div className="w-full flex justify-between items-baseline">
                    <p className="ml-4 text-base font-semibold leading-6 text-gray-900 dark:text-gray-400">{item.name}</p>
